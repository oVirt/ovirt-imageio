...
def test_method_not_allowed(env):
    with http.UnixClient(env.cfg.tickets.socket) as c:
...
def test_get_not_found(env):
    with http.UnixClient(env.cfg.tickets.socket) as c:
        res = c.request("GET", "/tickets/%s" % uuid.uuid4())
        assert res.status == 404


def test_put(env, fake_time):
...
def test_put_bad_url_value(env, fake_time):
    ticket = testutil.create_ticket(url='http://[1.2.3.4:33')
    body = json.dumps(ticket)
    with http.UnixClient(env.cfg.tickets.socket) as c:
        res = c.request("PUT", "/tickets/%(uuid)s" % ticket, body)
        assert res.status == 400
        pytest.raises(KeyError, env.auth.get, ticket["uuid"])


def test_general_exception(env, monkeypatch):
    def fail(*a, **kw):
...
def test_put_mandatory_fields(env, missing):
    ticket = testutil.create_ticket()
...
def test_put_invalid_timeout(env):
    ticket = testutil.create_ticket()
...
def test_put_url_type_error(env):
    ticket = testutil.create_ticket()
    ticket["url"] = 1
    body = json.dumps(ticket)
    with http.UnixClient(env.cfg.tickets.socket) as c:
...
def test_put_url_scheme_not_supported(env):
    ticket = testutil.create_ticket()
...
def test_extend_negative_timeout(env):
...
def test_get_expired_ticket(env, fake_time):
    ticket = testutil.create_ticket()
    env.auth.add(ticket)
...
def test_extend_expired_ticket(env, fake_time):
    ticket = testutil.create_ticket()
    env.auth.add(ticket)
    # Make the ticket expire.
    fake_time.now += 500
...
def test_extend_no_ticket_id(env, fake_time):
    ticket = testutil.create_ticket()
    env.auth.add(ticket)
    prev_ticket = env.auth.get(ticket["uuid"]).info()
    body = json.dumps({"timeout": 300})
    with http.UnixClient(env.cfg.tickets.socket) as c:
        res = c.request("PATCH", "/tickets/", body)
        cur_ticket = env.auth.get(ticket["uuid"]).info()
        assert res.status == 400
        assert cur_ticket == prev_ticket


def test_extend_invalid_json(env, fake_time):
    ticket = testutil.create_ticket()
    env.auth.add(ticket)
    prev_ticket = env.auth.get(ticket["uuid"]).info()
    with http.UnixClient(env.cfg.tickets.socket) as c:
...
def test_extend_no_timeout(env, fake_time):
    ticket = testutil.create_ticket()
    env.auth.add(ticket)
    prev_ticket = env.auth.get(ticket["uuid"]).info()
...
def test_extend_invalid_timeout(env, fake_time):
    ticket = testutil.create_ticket()
    env.auth.add(ticket)
    prev_ticket = env.auth.get(ticket["uuid"]).info()
...
def test_idle_time_active(env, fake_time, tmpdir):
    ...
    with http.Client(env.cfg) as c:
        ...
        fake_time.now += 200
        assert env.auth.get(ticket["uuid"]).idle_time == 0


def test_idle_time_inactive(env, fake_time):
    ticket = testutil.create_ticket()
    env.auth.add(ticket)
    ...
    fake_time.now += 200
    assert env.auth.get(ticket["uuid"]).idle_time == 200


def test_idle_time_put(env, fake_time, tmpdir):
...
def test_idle_time_get(env, fake_time, tmpdir):
    image = testutil.create_tempfile(tmpdir, "image", b"a" * 8192)
    ticket = testutil.create_ticket(url="file://" + str(image))
    env.auth.add(ticket)
...
def test_idle_time_patch(env, fake_time, tmpdir, msg):
    image = testutil.create_tempfile(tmpdir, "image", b"a" * 8192)
    ticket = testutil.create_ticket(url="file://" + str(image))
    env.auth.add(ticket)
...
def test_idle_time_options(env, fake_time):
...
def test_delete_one_not_found(env):
    with http.UnixClient(env.cfg.tickets.socket) as c:
